(() => {
  ...
  const state = {
    ...
    accounts: [], content: [], metrics: [], syncRuns: [], ai: [], charts: [], loadedAt: null,
  };
  ...
  async function loadData(showMessage = true) {
    ...
    try {
      const [accounts, content, metrics, syncRuns, ai] = await Promise.all([
      ...
      Object.assign(state, { accounts, content, metrics, syncRuns, ai, loadedAt: new Date() });
      $("last-refresh").textContent = `Betöltve: ${dateTimeHU(state.loadedAt)}`;
      $("footer-data-note").textContent = `${num(content.length)} tartalom · ${num(metrics.length)} adatsor · ${num(ai.length)} AI rekord`;
      renderPage();
    } catch (error) {
  ...
  function renderObserver(){
    ...
    const topEntities=Object.entries(entities).sort((a,b)=>b[1]-a[1]),high=filtered.filter((x)=>contentPriority(x.c)>=4).length,aiReady=filtered.filter((x)=>x.a&&x.a.status==="success").length,reviewCount=filtered.filter((x)=>Boolean(x.a?.requires_review)).length;
    const highlighted=filtered.filter((x)=>x.a&&x.a.status==="success"&&contentPriority(x.c)>=4).sort((a,b)=>Number(b.a?.final_priority||0)-Number(a.a?.final_priority||0)||String(b.c.published_at).localeCompare(String(a.c.published_at))).slice(0,6);
    pageContent.innerHTML=`<section class="kpi-grid">${kpi("Médiatörténetek",num(filtered.length),"külön feldolgozott cikkek",p?delta(filtered.length,prevFiltered.length):undefined)}${kpi("Sajtómegjelenések",num(mentions),"elsődleges + hasonló megjelenések",p?delta(mentions,prevMentions):undefined)}${kpi("Különböző források",num(sources.size),"felismert médiadomének",p?delta(sources.size,prevSources.size):undefined)}${kpi("AI review-jelzés",num(reviewCount),`${num(high)} magas prioritású történet · ${num(aiReady)}/${num(filtered.length)} AI-feldolgozva`,p?delta(interviews,prevInterviews):undefined)}</section>
    <div class="callout"><strong>AI médiaintelligencia aktív</strong><p>A Cloudflare AI a beérkező Observer-történeteket röviden összefoglalja, témába sorolja, pontozza és javasolt teendőt ad. Jelenleg ${num(aiReady)} a ${num(filtered.length)} médiatörténetből már AI-val is fel van dolgozva ebben az időszakban.</p></div>
    ...
    <section class="grid-2" style="margin-top:15px"><article class="panel"><div class="panel-heading"><div><p class="eyebrow">AI FIGYELŐLISTA</p><h2>Most figyelendő megjelenések</h2></div><span class="panel-note">prioritás 4-5</span></div><div id="observer-ai-list" class="insight-list"></div></article><article class="panel"><div class="panel-heading"><div><p class="eyebrow">AI TÉMAKÖRÖK</p><h2>Leggyakoribb AI-topikok</h2></div></div><div id="observer-topic-rank" class="rank-list"></div></article></section>
    <article class="panel"><div class="panel-heading"><div><p class="eyebrow">SAJTÓADATBÁZIS</p><h2>Minden médiatörténet</h2></div><div class="table-tools"><input id="observer-search" type="search" placeholder="Keresés címben, összefoglalóban…"><select id="observer-type"><option value="all">Minden típus</option><option value="interjú">Interjú / megszólalás</option><option value="high">Magas prioritás</option><option value="review">AI review-jelzett</option></select></div></div><div class="table-wrap observer-table-wrap"><table id="observer-table" class="sortable-table observer-table"><colgroup><col class="obs-date"><col class="obs-title"><col class="obs-source"><col class="obs-entity"><col class="obs-depth"><col class="obs-count"><col class="obs-priority"></colgroup><thead><tr><th data-sort-type="date">Dátum</th><th data-sort-type="text">Cím és ismertetés</th><th data-sort-type="text">Forrás</th><th data-sort-type="text">Érintett entitás</th><th data-sort-type="text">Megjelenítés / AI téma</th><th class="num" data-sort-type="number">Megjelenések</th><th data-sort-type="number">Prioritás</th></tr></thead><tbody id="observer-body"></tbody></table></div><p id="observer-count" class="metric-definition"></p></article>`;
