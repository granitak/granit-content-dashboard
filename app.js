(() => {
  ...
  const PAGE_META = {
    ...
    youtube: ["VIDEÓ", "YouTube", "Részletes YouTube Analytics, közönségnövekedés és videóteljesítmény a friss Data API-snapshotokkal együtt."],
  ...
  function latestAvailableDate(source) {
    ...
    if(source==="youtube") return latestMetricDate(source,["watch_minutes"],true)||latestMetricDate(source,["views"],true)||latestContentDate(source);
  ...
  function youtubeIsoDurationSeconds(value) {
    const m=String(value||"").match(/^P(?:(\d+)D)?(?:T(?:(\d+)H)?(?:(\d+)M)?(?:(\d+(?:\.\d+)?)S)?)?$/i);
    if(!m)return 0;
    return Number(m[1]||0)*86400+Number(m[2]||0)*3600+Number(m[3]||0)*60+Number(m[4]||0);
  }
  function youtubeAnalyticsBounds() {
    const rows=state.metrics.filter((m)=>m.source==="youtube"&&!m.content_external_id&&m.metric_name==="watch_minutes");
    const dates=rows.map((m)=>String(m.metric_date||"").slice(0,10)).filter((d)=>/^\d{4}-\d{2}-\d{2}$/.test(d)).sort();
    if(!dates.length)return {first:null,last:null};
    return {first:new Date(`${dates[0]}T00:00:00`),last:new Date(`${dates.at(-1)}T23:59:59`)};
  }
  function youtubeAnalyticsRange(base=selectedRange()) {
    const bounds=youtubeAnalyticsBounds();
    if(!bounds.last)return base;
    let end=new Date(Math.min(base.end.getTime(),bounds.last.getTime()));
    let start=base.start?new Date(base.start):null;
    let days=base.days;
    if(rangeSelect.value==="all") {
      start=bounds.first?new Date(bounds.first):start;
      days=null;
    } else if(base.start && end<base.end) {
      if(rangeSelect.value==="year") {
        start=new Date(end.getFullYear(),0,1);start.setHours(0,0,0,0);
        days=Math.ceil((end-start)/86400000)+1;
      } else {
  ...
  function youtubeDailyMetricSeries(name, range=youtubeAnalyticsRange()) {
    const dates=allDates(range),map=Object.fromEntries(dates.map((d)=>[d,0]));
    state.metrics.filter((m)=>m.source==="youtube"&&!m.content_external_id&&m.metric_name===name&&inRange(m.metric_date,range)).forEach((m)=>{
      const d=dayKey(m.metric_date);if(d in map)map[d]=Number(m.metric_value||0);
    });
  ...
  function ensureYouTubeVisualPolish(){
    if(document.getElementById("youtube-visual-polish"))return;
    const style=document.createElement("style");
    style.id="youtube-visual-polish";
    style.textContent=`
      /* YouTube Phase 4.6 — compact intelligence layout */
      .youtube-intel-row{align-items:start;}
      .youtube-intel-row>.panel,.youtube-reach-grid>.panel{display:flex;flex-direction:column;min-height:0;overflow:hidden;}
      .youtube-intel-row .panel-heading,.youtube-reach-grid .panel-heading{flex:0 0 auto;}
      .youtube-scroll-list{min-height:0;overflow-y:auto;overflow-x:hidden;padding-right:7px;scrollbar-gutter:stable;}
      .youtube-scroll-list::-webkit-scrollbar,.youtube-video-table-wrap::-webkit-scrollbar{width:7px;height:7px;}
      .youtube-scroll-list::-webkit-scrollbar-track,.youtube-video-table-wrap::-webkit-scrollbar-track{background:transparent;}
      .youtube-scroll-list::-webkit-scrollbar-thumb,.youtube-video-table-wrap::-webkit-scrollbar-thumb{background:#cbd9d6;border-radius:999px;}
      ...
      .youtube-video-table-wrap{max-height:560px;overflow:auto;}
      .youtube-retention-chart{height:300px!important;}
  ...
  function renderYouTube() {
    ...
    <section class="grid-2 equal youtube-intel-row youtube-intel-row--standard">
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">FELFEDEZÉS</p><h2>Forgalmi források</h2></div><span class="panel-note">részesedés a megtekintésekből</span></div><div id="youtube-traffic-rank" class="rank-list youtube-scroll-list"></div></article>
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">KÖZÖNSÉG</p><h2>Feliratkozott vs. nem feliratkozott</h2></div><span class="panel-note">megtekintések megoszlása</span></div><div class="chart-wrap youtube-chart-fit"><canvas id="youtube-subscriber-mix"></canvas></div></article>
    </section>
    <section class="grid-2 equal youtube-intel-row youtube-intel-row--short" style="margin-top:15px">
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">ESZKÖZÖK</p><h2>Hol nézik a videókat?</h2></div><span class="panel-note">megtekintések eszköztípus szerint</span></div><div class="chart-wrap youtube-chart-fit"><canvas id="youtube-device-mix"></canvas></div></article>
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">FORMÁTUM</p><h2>Shorts, normál videó, live</h2></div><span class="panel-note">YouTube creatorContentType</span></div><div id="youtube-content-rank" class="rank-list youtube-scroll-list"></div></article>
    </section>
    ${(searchTerms.length||externalReferrers.length)?`<section class="grid-2 equal youtube-intel-row youtube-intel-row--lists" style="margin-top:15px">
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">KERESÉSI SZÁNDÉK</p><h2>YouTube keresési kifejezések</h2></div><span class="panel-note">top 25 · görgethető lista</span></div><div id="youtube-search-rank" class="rank-list youtube-scroll-list"></div></article>
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">KÜLSŐ FORGALOM</p><h2>Hivatkozó oldalak</h2></div><span class="panel-note">top találatok · görgethető lista</span></div><div id="youtube-external-rank" class="rank-list youtube-scroll-list"></div></article>
    </section>`:""}
    <section class="grid-2 equal youtube-intel-row youtube-intel-row--lists" style="margin-top:15px">
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">FÖLDRAJZ</p><h2>Legfontosabb országok</h2></div><span class="panel-note">top 10 · görgethető lista</span></div><div id="youtube-country-rank" class="rank-list youtube-scroll-list"></div></article>
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">TOP VIDEÓK</p><h2>Az időszak legerősebb videói</h2></div><span class="panel-note">időszaki Analytics · görgethető</span></div><div id="youtube-video-rank" class="rank-list youtube-scroll-list"></div></article>
    </section>
    ${(ageGroups.length||genders.length||sharing.length)?`<section class="grid-3 youtube-intel-row youtube-intel-row--three" style="margin-top:15px">
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">DEMOGRÁFIA</p><h2>Korosztály</h2></div><span class="panel-note">viewer percentage</span></div><div id="youtube-age-rank" class="rank-list youtube-scroll-list"></div></article>
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">DEMOGRÁFIA</p><h2>Nem</h2></div><span class="panel-note">viewer percentage</span></div><div class="chart-wrap youtube-chart-fit"><canvas id="youtube-gender-mix"></canvas></div></article>
      <article class="panel"><div class="panel-heading"><div><p class="eyebrow">MEGOSZTÁS</p><h2>Hol osztják tovább?</h2></div><span class="panel-note">görgethető lista</span></div><div id="youtube-sharing-rank" class="rank-list youtube-scroll-list"></div></article>
    </section>`:""}
    ${retentionOptions.length?`<article class="panel" style="margin-top:15px"><div class="panel-heading"><div><p class="eyebrow">KÖZÖNSÉGMEGTARTÁS</p><h2>Audience retention görbe</h2></div><div class="table-tools"><select id="youtube-retention-select">${retentionOptions.map((id)=>{const c=state.content.find((x)=>x.source==="youtube"&&String(x.external_id)===id);return `<option value="${esc(id)}">${esc(clampText(c?.title||id,70))}</option>`;}).join("")}</select></div></div><div class="chart-wrap youtube-retention-chart"><canvas id="youtube-retention-chart"></canvas></div><p class="metric-definition">Az audience watch ratio azt mutatja, hogy a videó adott pontján a megtekintések mekkora része van még jelen; 100% fölé is kerülhet visszatekerés/újranézés miatt. A relatív retention az azonos hosszúságú YouTube-videókhoz viszonyít.</p></article>`:""}`:`<div class="callout" style="margin-top:15px"><strong>A Phase 4.6 adatmodell még üres.</strong><p>A Supabase-migráció és a syncYouTubeAdvancedAnalytics() első sikeres futása után itt jelennek meg a forgalmi források, eszközök, földrajz, formátumok, demográfia és az időszaki videó-Analytics.</p></div>`;

    const reachPanels=reach.rows.length?`<div class="section-title" style="margin-top:18px"><div><p class="eyebrow">REACH</p><h2>Thumbnail impressions és CTR</h2></div><span class="panel-note">Reporting API · ${reachFresh?`${dateHU(reachFresh)}-ig`:""}</span></div><section class="grid-2 equal youtube-reach-grid"><article class="panel"><div class="panel-heading"><div><p class="eyebrow">MEGJELENÉS → KATTINTÁS</p><h2>Thumbnail reach trend</h2></div><span class="panel-note">${num(reach.impressions,true)} impressions · ${pct(reach.ctr)} súlyozott CTR</span></div><div class="chart-wrap youtube-chart-fit"><canvas id="youtube-reach-chart"></canvas></div></article><article class="panel"><div class="panel-heading"><div><p class="eyebrow">THUMBNAIL TELJESÍTMÉNY</p><h2>Legnagyobb impressions videók</h2></div><span class="panel-note">CTR-rel együtt · görgethető</span></div><div id="youtube-reach-rank" class="rank-list youtube-scroll-list"></div></article></section>`:`<div class="callout" style="margin-top:15px"><strong>Thumbnail Reach előkészítve.</strong><p>A YouTube Reporting API új reach riportja a reporting job létrehozása után jellemzően 24–48 órával kezd adatot adni. Amint megérkezik, itt automatikusan megjelenik az impressions, a thumbnail CTR, a napi trend és a videónkénti reach.</p></div>`;
    ...
    lineChart("youtube-views-chart",viewSeries.labels,[
      {label:"Megtekintések",data:viewSeries.values,borderColor:"rgba(217,45,32,.38)",backgroundColor:"rgba(217,45,32,.08)",fill:true,pointRadius:0},
  ...
  function platformConfig(source) {
    const configs={
      ...
      youtube:{cards:[["Megtekintések","exposure"],["Interakciók","engagement"],["Feliratkozók","audience"],["Összes csatornamegtekintés","channel_views"],["Videók száma","video_count"],["Publikált videók","publishing"]],note:"YouTube Data API nyilvános adatai. Nézési idő és pontos napi feliratkozóváltozás OAuth nélkül nem érhető el."},
  ...
  function renderPlatform(source) {
    if(source==="youtube"){renderYouTube();return;}
