(() => {
  ...
  const SOURCE = {
    ...
    mailchimp: { label: "Hírlevél", short: "Hírlevél", color: "#13707d", exposure: ["delivered", "emails_sent"], clicks: ["unique_clicks", "clicks"], engagement: ["unique_clicks"], audience: ["audience_members"] },
  ...
  function deterministicInsights() {
    ...
    if(scored[0]) insights.push({title:`A legnagyobb aktivitást a ${sourceLabel(scored[0].s)} adta`,text:`Az időszakban ${num(scored[0].value)} platformnatív megjelenést vagy megtekintést mértünk ezen a csatornán.`,type:"normal"});
    if(top) insights.push({title:"A legerősebb tartalom",text:`„${clampText(top.c.title,130)}” – ${num(top.s.exposure)} megjelenés/megtekintés, ${num(top.s.clicks)} kattintás és ${num(top.s.engagement)} interakció.`,type:"normal"});
  ...
  function renderOverview() {
    ...
    const mail=sourceMetric("mailchimp","clicks",r), mailPrev=p?sourceMetric("mailchimp","clicks",p):0;
    ...
    pageContent.innerHTML=`<section class="kpi-grid six">${kpi("Blogmegtekintések",num(web),"GA4 / kiválasztott időszak",delta(web,webPrev))}${kpi("Hírlevél-kattintások",num(mail),"egyedi kattintások",delta(mail,mailPrev))}${kpi("Social megjelenések",num(social,true),"platformnatív mutatók összege",delta(social,socialPrev))}${kpi("Követők és feliratkozók",num(aud,true),"legfrissebb állapot")}${kpi("Megjelent tartalmak",num(pub),"cikkek, posztok, videók, levelek",delta(pub,pubPrev))}${kpi("Sajtómegjelenések",num(mentions),"Observer-megjelenések",delta(mentions,mentionsPrev))}</section>
  ...
  function renderNewsletter() {
    ...
    const metricSum=(items,name)=>items.reduce((s,x)=>s+contentMetric(x.c,[name]),0);
    const sent=metricSum(rows,"emails_sent"), delivered=metricSum(rows,"delivered"), opens=metricSum(rows,"unique_opens"), clicks=metricSum(rows,"unique_clicks"), unsub=metricSum(rows,"unsubscribes"), bounce=metricSum(rows,"hard_bounces")+metricSum(rows,"soft_bounces");
    const pClicks=metricSum(prevRows,"unique_clicks");
    pageContent.innerHTML=`<section class="kpi-grid six">${kpi("Kampányok",num(rows.length),rangeLabel())}${kpi("Kézbesített levelek",num(delivered),`${delivered&&sent?pct(delivered/sent):"–"} kézbesítési arány`)}${kpi("Egyedi megnyitók",num(opens),`${delivered?pct(opens/delivered):"–"} megnyitási arány`)}${kpi("Egyedi kattintók",num(clicks),`${delivered?pct(clicks/delivered):"–"} kattintási arány`,delta(clicks,pClicks))}${kpi("Leiratkozások",num(unsub),delivered?pct(unsub/delivered):"–")}${kpi("Feliratkozók",num(audience("mailchimp")),"aktuális listaállomány")}</section>
    <div class="callout"><strong>Adatértelmezési megjegyzés</strong><p>A megnyitási arányt az Apple Mail adatvédelmi funkciói torzíthatják. A kattintás, az átkattintási arány és a weboldalon mért viselkedés megbízhatóbb teljesítménymutató.</p></div>
    <section class="grid-2 equal" style="margin-top:15px"><article class="panel"><div class="panel-heading"><div><p class="eyebrow">KAMPÁNYTREND</p><h2>Megnyitási és kattintási arány</h2></div></div><div class="chart-wrap"><canvas id="mail-rate-chart"></canvas></div></article>
    <article class="panel"><div class="panel-heading"><div><p class="eyebrow">KATTINTÁSOK</p><h2>Legsikeresebb kampányok</h2></div></div><div id="mail-rank" class="rank-list"></div></article></section>
    <article class="panel"><div class="panel-heading"><div><p class="eyebrow">HAVI KIMUTATÁS</p><h2>Hírlevél-statisztika</h2></div><span class="panel-note">A korábbi Excel-logika automatikusan előállítva</span></div><div class="table-wrap"><table><thead><tr><th>Dátum</th><th>Cím</th><th class="num">Címzettek</th><th class="num">Kézbesítés</th><th class="num">Megnyitás</th><th class="num">Kattintás</th><th>Legtöbbet kattintott link</th><th>Legsikeresebb Grandio-cikk</th><th class="num">Leiratkozás</th></tr></thead><tbody>${rows.length?rows.map(({c,m})=>{const s=contentMetric(c,["emails_sent"]),d=contentMetric(c,["delivered"]),o=contentMetric(c,["unique_opens"]),cl=contentMetric(c,["unique_clicks"]),u=contentMetric(c,["unsubscribes"]);return `<tr><td>${dateHU(c.published_at)}</td><td><a class="content-link" data-content="mailchimp|${esc(c.external_id)}" href="#">${esc(c.title)}</a><div class="metric-definition">${esc(m.subject_line||"")}</div></td><td class="num">${num(s)}</td><td class="num">${num(d)}<div class="metric-definition">${s?pct(d/s):"–"}</div></td><td class="num">${num(o)}<div class="metric-definition">${d?pct(o/d):"–"}</div></td><td class="num">${num(cl)}<div class="metric-definition">${d?pct(cl/d):"–"}</div></td><td>${m.top_link_url?`<a class="content-link" target="_blank" rel="noopener" href="${esc(m.top_link_url)}">${esc(clampText(m.top_link_url,55))}</a><div class="metric-definition">${num(m.top_link_unique_clicks||m.top_link_clicks)} kattintás</div>`:"–"}</td><td>${m.top_grandio_url?`<a class="content-link" target="_blank" rel="noopener" href="${esc(m.top_grandio_url)}">${esc(clampText(m.top_grandio_url,55))}</a><div class="metric-definition">${num(m.top_grandio_unique_clicks||m.top_grandio_clicks)} kattintás</div>`:"–"}</td><td class="num">${num(u)}</td></tr>`;}).join(""):`<tr><td colspan="9"><div class="empty-state"><strong>Nincs kampány ebben az időszakban.</strong></div></td></tr>`}</tbody></table></div></article>`;
    const chronological=[...rows].reverse(); lineChart("mail-rate-chart",chronological.map((x)=>dateHU(x.c.published_at)),[{label:"Megnyitási arány",data:chronological.map((x)=>{const d=contentMetric(x.c,["delivered"]),o=contentMetric(x.c,["unique_opens"]);return d?o/d*100:0;}),borderColor:"#13707d",tension:.3},{label:"Kattintási arány",data:chronological.map((x)=>{const d=contentMetric(x.c,["delivered"]),c=contentMetric(x.c,["unique_clicks"]);return d?c/d*100:0;}),borderColor:"#2de68c",tension:.3}],{scales:{x:{grid:{display:false}},y:{beginAtZero:true,ticks:{callback:(v)=>`${v}%`}}}});
    const sorted=[...rows].sort((a,b)=>contentMetric(b.c,["unique_clicks"])-contentMetric(a.c,["unique_clicks"])).slice(0,7); const max=Math.max(...sorted.map((x)=>contentMetric(x.c,["unique_clicks"])),1); $("mail-rank").innerHTML=sorted.map((x,i)=>{const v=contentMetric(x.c,["unique_clicks"]);return `<div class="rank-row"><span class="rank-index">${i+1}</span><div class="rank-title">${esc(clampText(x.c.title,65))}<small>${dateHU(x.c.published_at)}</small><div class="progress"><span style="width:${v/max*100}%"></span></div></div><span class="rank-value">${num(v)}</span></div>`;}).join("")||`<div class="empty-state">Nincs adat.</div>`;
  }
